...
def close_cash_drawer(win, status="close"):
    ...
    try:
        ...
        if status == "close":
            win.child_window(control_type="Button", title="Log SO").click_input()
            if cash_btn.is_enabled():
                print("▶️ Clicking 'Close Drawer' button to close the cash drawer.")
                logger.log("▶️ Clicking 'Close Drawer' button to close the cash drawer.", status="pass")
                ...
                logger.log("ℹ️ 'Close Drawer' button is not enabled (already closed).", status="fail")
                logger.take_screenshot("Close_Drawer_button_not_enabled")
                return False
        else:
