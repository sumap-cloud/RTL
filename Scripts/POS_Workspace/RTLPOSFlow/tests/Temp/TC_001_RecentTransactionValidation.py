...
def test_basic_loyalty_flow():
    ...
    try:
        ...
        # Step 5: Reprint receipt via toggle menu (always runs)
        print("\n--- Step 5: Reprinting receipt via toggle menu (always runs) ---")
        try:
            # Use centralized ReprintReceiptFlow from utils
            try:
            ...
            if not rep.open_reprint_and_search():
                ...
                try:
                    ...
                    print(f"Exception extracting Store/POS/Trans values via ReprintReceiptFlow: {ex}")

            # Ensure we close the view
            try:
                if rep.click_cancel():
                    print("Clicked Cancel via ReprintReceiptFlow.")
                else:
                    print("Cancel button not found or click failed via ReprintReceiptFlow.")
            except Exception as e:
                print(f"Exception while attempting Cancel via ReprintReceiptFlow: {e}")
