...
# from RTLPOSFlow.utils.Login import Login
...
# print(f"{Path(__file__).parent.parent}/resources")
# parent_dir = f"{Path(__file__).parent.parent}/resources"
...
#     csv_path = f"{parent_dir}/{DATA_FILE}"
...
#     csv_path = f"{parent_dir}/{TRANSACTION_DATA_FILE}"
#     temp_path = Path(csv_path).with_suffix('.tmp')
#     try:
#         rows = []
#         with open(csv_path, mode='r', newline='', encoding='utf-8') as file:
...
#     try:
#         login_page = Login()
#         app1 = login_page.initialize_and_login(".*R10PosClient.*", TEST_CONFIG["credentials"]["username"], TEST_CONFIG["credentials"]["password"])

#         if app1:
#             # ADD found_index=0 to resolve the ElementAmbiguousError
#             nosale_btn = app1.child_window(title="No Sale", control_type="Button", found_index=0)
#             void_btn = app1.child_window(title="Void Transaction", control_type="Button", found_index=0)

#             print("Waiting for dashboard to stabilize...")

#             rr = app1.child_window(title="Item Search", control_type="Button")
#             if rr.exists(timeout=5):
#                 print("HI")


#             if nosale_btn.exists(timeout=12): 
#                 print("✅ Success: At Dashboard.")
#             elif void_btn.exists():
#                 print("Detected Void state. Processing approval...")
#                 void_btn.click_input()
#                 login_page.approval_popup_handler("AtMgr5", "abcd1234")
                
#                 # Check No Sale again after Void sequence
...
from typing import Tuple, Optional
from pywinauto import Application
from pywinauto.application import WindowSpecification  # Add this line
...
# try:
    ...
    from RTLPOSFlow.utils.ReprintReceipt import ReprintReceiptFlow
    from RTLPOSFlow.utils.Login import Login
    from RTLPOSFlow.utils.Enter_loyalty_card import handle_customer_popup
...
def read_scenario_data() -> Optional[dict]:
    csv_path = RESOURCE_DIR / DATA_FILE
    try:
        with open(csv_path, mode='r', newline='', encoding='utf-8') as file:
            reader = csv.DictReader(file)
            for row in reader:
                if (row['Banner'].strip() == BANNER_NAME and 
                    ...
                    return {
                        'username': row['Username'],
                        'password': row['Password'],
                        'ean_codes': [code.strip() for code in row['Item_EAN'].split(',')],
                        'loyalty_card': row['Card_number'],
                        'payment_method': row['Payment_method']
                    }
        print(f"❌ No matching row found in {DATA_FILE}")
        return None
    except Exception as e:
        print(f"❌ Error reading data: {e}")
        return None

def update_scenario_status(status: str, remarks: str = ""):
    csv_path = RESOURCE_DIR / DATA_FILE
...
def update_transaction_logging(store_no=None, pos_no=None, trans_no=None, amount=None):
    csv_path = RESOURCE_DIR / TRANSACTION_DATA_FILE
    temp_path = csv_path.with_suffix('.tmp')
    try:
        rows = []
        with open(csv_path, mode='r', newline='', encoding='utf-8') as file:
        ...
        for row in rows:
            if (row['Banner'].strip() == BANNER_NAME and 
                row['TC_ID'].strip() == SCRIPT_TC_ID and 
                str(row['Iteration']).strip() == str(ITERATION)):
                if store_no: row['Store_No'] = store_no
                if pos_no: row['Pos_No'] = pos_no
                if trans_no: row['Transaction_No'] = trans_no
                if amount: row['TransactionAmount'] = amount
                row['Transaction_Data'] = datetime.now().strftime("%Y-%m-%d")
                updated = True
...
def perform_login() -> Tuple[Optional[Application], Optional[WindowSpecification]]:
    """
    Perform login to the POS application.
    """
    login_page = Login()
    app = login_page.initialize_and_login(".*R10PosClient.*", "Atcash1", "abcd1234")

    try:
        # app = Login.initialize_and_login(".*R10PosClient.*", "Atcash1", "abcd1234")

        if app:
            nosale_btn = app.child_window(title="No Sale", control_type="Button", found_index=0)
            void_btn = app.child_window(title="Void Transaction", control_type="Button", found_index=0)
        ...
        # 3. Connect to the active POS session
        app = Application(backend="uia").connect(title_re=TEST_CONFIG["window_title"], timeout=20)
        win = app.window(title_re=TEST_CONFIG["window_title"])
        win.set_focus()

        # 4. Verify Dashboard Stability
        nosale_btn = win.child_window(title="No Sale", control_type="Button", found_index=0)
        if nosale_btn.exists(timeout=15):
...
if __name__ == "__main__":
    ...
    start_time = datetime.now()
    test_passed = test_basic_loyalty_flow()
    duration = (datetime.now() - start_time).total_seconds()

    if test_passed:
        print(f"⭐⭐⭐ Test Passed in {duration:.2f}s")
        update_scenario_status("Completed", f"Success in {duration:.2f}s")
    else:
        print("🛑 Test Failed")
        update_scenario_status("Failed", "Execution interrupted. Check logs.")
        sys.exit(1)