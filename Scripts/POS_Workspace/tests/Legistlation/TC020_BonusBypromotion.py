...
def test_BonusBuy():
    ...
    if not validate_loyalty_card():
        print("\n Failed to get Customer Screen Info")
        return False
    ...
    if not process_tender(tender_name="Goodwill Gesture"):
        print("\n Failed to select Goodwill Gesture")
        return False
    time.sleep(5)

    
    print("\n>>> Running with completely custom reasons and click target...")
    custom_reasons = {"Seasonal Event", "Customer Complaint"}
    handle_select_reason_code_popup(
         app, 
         expected_reasons=custom_reasons, 
         reason_to_click="Customer Complaint"
     )


    if not handle_Any_popup(specific_button="OK"):
