...
from Components.Loyalty.PromotionSelectDetail import scrape_promotion_details
...
def test_BonusBuy():
    ...
    if not toggle_menu_navigate(["Promotion Enquiry"]):
        print("\n Failed to click Promotional Enquiry")
        return False
    

    
    if not scrape_promotion_details():
        print("\nScript finished Details Failed to retrieve.")
        return False

    time.sleep(2)

    click_OK_button = win.child_window(title="Cancel", control_type="Button")
