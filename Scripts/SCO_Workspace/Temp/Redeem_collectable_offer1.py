...
def redeem_collectable_offer(offer_type="Collectable", collectable_offer_list=None):
    ...
    try:
        ...
        print(f"{offer_type} offer(s) to redeem: {offers}")
        ...
        for offer in offers:
            ...
            if not msg:
                print(f"❌ Could not read any Text content from {offer_type} popup. "
                      f"Listing descendants for debug:")
                try:
            ...
            if not collectable_btn.exists(timeout=1):
                print(f"❌ {offer_type} offer button '{offer_btn_title}' "
                      f"not found on popup.")
                continue
            ...
            if matched:
                ...
                print(f"✅ Handled {offer_type} offer '{clean_msg}' "
                      f"with action '{btn_label}'.")
                global_instance.reward_redeem_status = True
            else:
                print(f"❌ {offer_type} offer message did not match expected "
                      f"'{offer_msg}'.")
                continue
