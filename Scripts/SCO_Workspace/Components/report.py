...
class HTMLTestLogger:
    ...
    def upgrade_info_to_pass(self, substring):
        """
        Retroactively flip any 'info' log entries whose action contains
        `substring` to 'pass'. Call this after EE verification succeeds
        so that "No Exciting News prompt detected" turns green when the
        EE log confirms the transaction was valid.
        """
        for entry in self.entries:
            if entry.get("status", "").lower() == "info" and substring.lower() in entry.get("action", "").lower():
                entry["status"] = "pass"
                entry["action"] = entry["action"].replace("ℹ️", "✅")

    def log_section(self, title):
        """Insert a full-width section header row into the HTML report."""
        entry = {
            "time": datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3],
            "action": title,
            "element": "",
    ...
    def save(self):
        ...
        # Count steps (exclude section headers from totals)
        real_entries = [e for e in self.entries if e.get("status", "").lower() != "section"]
        total_steps = len(real_entries)
        failed_steps = sum(1 for e in real_entries if e.get("status", "").lower() == "fail")
...
<head>
    ...
    <style>
        ...
        .summary-bar {{ background: #fff; margin: 10px auto 18px auto; max-width: 900px; border-radius: 8px; box-shadow: 0 2px 8px #0001; display: flex; justify-content: center; align-items: center; gap: 32px; padding: 14px 0; font-size: 1.1em; }}
        .summary-bar .total {{ color: #283593; font-weight: bold; }}
        .summary-bar .failed {{ color: {failed_color}; font-weight: bold; }}
        ...
        .section {{ background: #1a237e !important; }}
        .section td {{ color: #fff; font-weight: bold; font-size: 1.08em; padding: 11px 16px; letter-spacing: 0.03em; }}
...
<body>
    ...
    <div class="badge-container">
        <span class="overall-badge {badge_class}">{badge_text}</span>
    </div>
    <div class="summary-bar">
        <span class="total">Total Steps: {total_steps}</span>
        <span class="failed">Failed Steps: {failed_steps}</span>
        <span class="passed">Passed Steps: {passed_steps}</span>
    </div>
    <table>
    ...
    def _row(self, entry):
        if entry.get("status", "").lower() == "section":
            return f'<tr class="section"><td colspan="5">{entry["action"]}</td></tr>'
        img_link = ""
        if entry["screenshot"]:
