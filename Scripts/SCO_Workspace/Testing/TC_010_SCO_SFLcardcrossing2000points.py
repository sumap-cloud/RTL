"""
TC_010_SCO_SFLcardcrossing2000points.py
----------------------------------------
Test Case: Validation of POS/SCO transaction with SFL card (segment 109)
crossing 2000 points.

Scenario:
    Verify that the points are moved over 2000 points for an SFL (Save For
    an Occasion) segment card. The Exciting News prompt appears after
    moving to tender mode.

Pre-requisite:
    SFL card (segment 109) preconditioned to JUST UNDER 2000 points.
    ⚠️  Update Card_number in SaleData.csv before running.

Expected on-screen message at step 5:
    "YOUR SAVINGS ARE GROWING. You now have $xx to save for your occasion.
...
# NZ only — SFL card (segment 109), spend >$20 to earn enough points to cross 2000
# Card: 9490000001137
...
CARD_CODE = _get_value("Card_number", "9490000001137")
...
try:
    ...
    # Step 5: Verify "YOUR SAVINGS ARE GROWING" prompt.
    # move_to_tendermode() handles the prompt inline if it appears during the
    # tender transition. verify_exciting_news_prompt() acts as safety net.
    found = verify_exciting_news_prompt(timeout_seconds=15)
    if found:
        logger.log(
            "✅ Step 5 — 'YOUR SAVINGS ARE GROWING' (SFL savings) prompt verified.",
            status="pass",
        )
    else:
        logger.log(
    ...
    if not ee_result["all_passed"]:
        ...
        logger.log(
            "✅ TC_010 PASSED — SFL card crossed 2000 pts; transaction settled in EagleEye.",
            status="pass",
        )

except Exception as e:
    logger.log(f"❌ Unexpected error in TC_010: {e}", status="fail")
    print(f"❌ TC_010 ERROR: {e}")
    logger.take_screenshot("TC_010_Unexpected_Error")
