"""
TC_006_SCO_Temporarycardcrossing2000points.py
---------------------------------------------
Test Case: Validation of POS/SCO transaction with a Temporary card crossing
2000 points.

Scenario:
    Verify that the points are moved over 2000 points for a Temporary
    (unregistered) EDR card. The "Great news – you could save $xx ..."
    prompt is displayed after moving to tender mode. The card is added
    to the sale as UNREGISTERED. Transaction must settle in EagleEye.

Pre-requisite:
    Temporary EDR card preconditioned to JUST UNDER 2000 points.

Expected on-screen message at step 5:
    "Great news – you could save $xx off future shop! Join FREE before
     your card expires to enjoy your savings."

Steps automated:
    1. Login to POS.
    2. Scan articles (5 × Tim Tam = ~$12.00).
    3. Scan EDR card in Sale mode (added as unregistered).
    4. Move to tender mode.
    5. Verify "Great news ..." prompt (OCR) and dismiss.
...
CARD_CODE = _get_value("Card_number", "9344450008836")
...
try:
    ...
    # Temporary cards are added as unregistered — same scan_loyalty_salemode
    # is used; the SCO software classifies the card and shows the
    # appropriate prompt at tender time.
    ...
    if found:
        logger.log("✅ Step 5 — 'Great news ...' prompt verified for Temporary card.", status="pass")
    else:
        logger.log(
            "ℹ️ Step 5 — 'Great news ...' prompt was NOT detected within timeout. "
    ...
    if not ee_result["all_passed"]:
        ...
        logger.log(
            "✅ TC_006 PASSED — Temporary card crossed 2000 pts; transaction settled in EagleEye.",
            status="pass",
        )

except Exception as e:
    logger.log(f"❌ Unexpected error in TC_006: {e}", status="fail")
    print(f"❌ TC_006 ERROR: {e}")
    logger.take_screenshot("TC_006_Unexpected_Error")
