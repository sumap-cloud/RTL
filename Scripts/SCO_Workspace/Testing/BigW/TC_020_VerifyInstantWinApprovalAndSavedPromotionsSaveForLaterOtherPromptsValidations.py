"""
TC_020_VerifyInstantWinApprovalAndSavedPromotionsSaveForLaterOtherPromptsValidations.py
---------------------------------------------------------------------------------------
TC_020 — IW Approval Save-For-Later + Saved + Other Prompts
...
try:
    ...
    logger.log("  TC_020 — Instant Win Approval (SAVE FOR LATER) + Saved + others", status="info")
    logger.log("=" * 70, status="info")
    ...
    SAVED_DEN  = _get("Instant_win_offer_redeem", 1, "10")
    ...
    # Step A: IW APPROVAL — click SAVE FOR LATER.
    if not handle_instant_win_approval(action="save_for_later", timeout=20):
        logger.log("WARN — IW approval popup not handled or not present.", status="info")

    # Step B: SAVED-promotions popup — pick a denomination per scenario.
    ...
    if ee["all_passed"]:
        logger.log("PASS — TC_020 EE settled.", status="pass")
    else:
    ...
    logger.log(f"FAIL TC_020 unexpected error: {e}", status="fail")
    logger.take_screenshot("TC_020_Unexpected_Error")
