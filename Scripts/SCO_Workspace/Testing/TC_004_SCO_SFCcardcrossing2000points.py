"""
TC_004_SCO_SFCcardcrossing2000points.py
---------------------------------------
Test Case: Validation of POS/SCO transaction with SFC card (segment 105)
crossing 2000 points.

Scenario:
    Verify that the points are moved over 2000 points for an SFC-segment
    Registered EDR card and that the "Exciting News" prompt (Christmas
    savings variant) is displayed after moving to tender mode. Transaction
    must settle in EagleEye.

Pre-requisite:
    SFC card (segment 105) preconditioned to JUST UNDER 2000 points so
    this transaction crosses the threshold.

Expected on-screen message at step 5:
    "Exciting News! You've just banked $xx into your Christmas savings."
...
CARD_CODE = _get_value("Card_number", "9355215896100")
...
try:
    ...
    if found:
        logger.log("✅ Step 5 — Exciting News (Christmas savings) prompt verified.", status="pass")
    else:
        logger.log(
    ...
    if not ee_result["all_passed"]:
        ...
        logger.log(
            "✅ TC_004 PASSED — SFC card crossed 2000 pts; transaction settled in EagleEye.",
            status="pass",
        )

except Exception as e:
    logger.log(f"❌ Unexpected error in TC_004: {e}", status="fail")
    print(f"❌ TC_004 ERROR: {e}")
    logger.take_screenshot("TC_004_Unexpected_Error")
